...
| **H6** | **PHÉP ĐO DỨT ĐIỂM, chạy trước mọi row khác còn lại** — chạy **v3-r16 (=HF v4)** trên đúng **228 segment `source: youtube`** của tier1 test, rồi so với con số v4-mixed-r16 (=HF v5) đã có: **CER 0.0764**, retention (H3). Đây là bộ test duy nhất vừa là **speech thật**, vừa có **nhãn người soát**, vừa **cùng domain với audio production** (phỏng vấn tuyển dụng kỹ thuật). Hiện `gate_results.json` của v4-mixed-r16 chỉ so 0.0764 với baseline PhoWhisper (0.1593), **không** so với v4 — nên chưa ai biết v4 đạt bao nhiêu trên đó. Lấy adapter từ `Outputs/v3-r16/adapter/`, λ=0.5, chạy qua `src.gate._eval_split` với đúng `eval` config của v4-mixed-r16 để hai số so được | GPU Kaggle T4 (chỉ inference, không train) | — | `predictions_v3-r16_youtube228.csv` + CER và retention trên 228 segment đó | **Cổng phân nhánh cho toàn bộ phần còn lại.** v4 CER **thấp hơn** 0.0764 → v5 hồi quy thật trên domain production, và việc sửa data (D1/D3) có cơ sở. v4 CER **cao hơn** → v5 tốt hơn trên domain production, triệu chứng module 8 là vấn đề **casing convention** chứ không phải chất lượng phiên âm, và D3 (đổi tỉ lệ trộn) mất phần lớn lý do tồn tại. **Không** đọc kết quả này bằng cách so với baseline PhoWhisper — phép so duy nhất có nghĩa ở đây là v4 với v5 | **done — chạy trên Kaggle 2026-08-17, kết quả PHÂN ĐÔI, không đọc thành "v5 thắng sạch".** v3-r16 (λ=0.5, `Outputs/v3-r16/checkpoints/best` + `set_lambda`) trên đúng 228 segment: **CER 0.1205**, retention **0.7746**. v4-mixed-r16 cùng 228 segment (đã có từ trước): **CER 0.0764**, retention **0.7094**. Hai chỉ số **trái chiều nhau**: CER nói v5 tốt hơn (0.0764 < 0.1205), retention nói v5 tệ hơn (0.7094 < 0.7746, tức v5 làm mất từ mượn tiếng Anh nhiều hơn v3 — đúng chiều với triệu chứng production `team`→`tim` bị báo cáo, không phải nhiễu). Đọc thẳng theo cổng dừng 4 (chỉ khoá vào CER) sẽ kết luận "dừng D3" — nhưng cổng dừng 4 viết trước khi H3 tồn tại, và **lý do H3 được xây chính là vì CER một mình không bắt được kiểu lỗi mất từ mượn này** (xem đầu mục H3). Áp cổng dừng 4 nguyên văn ở đây là bỏ qua đúng triệu chứng ban đầu được báo cáo. **Chưa tự quyết row nào tiếp theo (H4b/D1/D3) — cần người dùng xác nhận đọc kết quả phân đôi này thế nào trước khi đi tiếp**, vì D3 tốn ~6.5h Kaggle GPU và quyết định đó không nên tự chọn âm thầm (CLAUDE.md §1). `predictions_v3-r16_youtube228.csv` đã ghi tại `D:\Fine_tune_wf\Outputs\predictions_v3-r16_youtube228.csv`. [scripts/eval_v3_on_youtube_test.py](scripts/eval_v3_on_youtube_test.py) và [notebooks/eval-v3-on-youtube-test.ipynb](notebooks/eval-v3-on-youtube-test.ipynb) chạy đúng như thiết kế, không cần sửa gì trên Kaggle — dự đoán "code Kaggle không chạy đúng lần đầu" (memory) sai lần này. |
| **H2** | **Đo lại N3 + phân đôi N2** — (a) tự đếm lại 4 chỉ số của handoff trên `artifacts/web-1786700346805/04_asr.json` và `.../web-1786942786371/04_asr.json`, xác nhận audio đầu vào hai run đồng nhất (so `01_*`/`02_*`); (b) chạy v5 với `prompt_ids` = `processor.get_prompt_ids("<glossary có casing>")` truyền vào `generate()` trong [remote_asr_server.py](d:/viet-speech/scripts/remote_asr_server.py), A/B trên **cùng file audio**. **Hạ ưu tiên 2026-08-17:** phần (b) chỉ đáng chạy nếu H6 xác nhận có hồi quy thật, vì nếu không thì không có gì cần "cứu" | GPU (Kaggle, model chạy remote) | H6 | Bảng số tự đo cho (a); transcript A/B cho (b) | **Cổng phân đôi N2.** Prompt phục hồi được `team`/`build` → thông tin còn trong weights, lệch prior, sửa ở decode là đủ. Không phục hồi → đã nướng vào weights. Rủi ro đã biết: prompt conditioning của Whisper dễ gây lặp/hallucinate — nếu output lặp thì đó là **fail của phép thử**, không phải kết quả âm tính; báo và đổi cách seed. Nếu (a) lệch số handoff thì dừng, N3 thành vấn đề chính | **(a) done, N3 ĐÓNG THẬT, kèm phát hiện quan trọng hơn cả acceptance ban đầu (2026-08-17).** Đếm lại 4 chỉ số trên đúng 2 file `04_asr.json` (mtime không đổi nội dung — số khớp handoff tuyệt đối): **8515 vs 8608** ký tự, chữ hoa **15→1**, `team` **7→1** (`tim`×5), `build` **2→0** (`bill`×2), `developer/standard/realize/nus` **1/1/1/2 → 0/0/0/0**. `01_vad.json`/`02_overlap.json` byte-identical giữa hai run → cùng audio đầu vào, N3 đóng dứt điểm. **Phát hiện mới, không nằm trong acceptance ban đầu:** `D:\viet-speech\demo\youtube_meeting\ground_truth.txt` là **transcript người soát tay thật** cho đúng file `first10.wav` (nguồn: `experiments/real-domain-gap-diagnosis/ground_truth.py` — hạ tầng có sẵn từ phiên trước, không phải tự dựng), cắt đúng **596.0s** (`YOUTUBE_CUT_S`, hằng số đã có sẵn, đúng bằng điểm cắt `first10.wav`) vì quá mốc đó bản chép loãng thành tóm tắt. Video nguồn `E5dAymt68-0` **không nằm trong 7 meeting train** `youtube-meetings` (đã đối chiếu `dataset/youtube-meetings/raw/`) — audio thật, tham chiếu người soát thật, **held-out khỏi cả hai model**, cùng domain phỏng vấn/webinar kỹ thuật với audio production. Ghép `07_transcript.json` theo `start` thành transcript liên tục mỗi model, qua đúng `Normalizer`/`score`/`english_token_retention` của pipeline này: **CER v4=0.2461 vs v5=0.2818 (v5 tệ hơn, có reference thật)**, retention **v4=0.7179 vs v5=0.5128** (mất `google`×6 `team`×6 `engineer`×4 `webinar`×3 `client`×2 `build`×2 — đúng chiều triệu chứng). Đây là bằng chứng nặng nhất trong toàn bộ điều tra: CER **và** retention cùng chiều "v5 tệ hơn" trên audio thật ngoài tập train của cả hai model — khác H6 (chỉ retention xấu đi, CER lại tốt hơn, trên slice thuộc miền train của v5). Khả năng cao nhất: H6 đo trên slice v5 **đã học quen** (youtube-meetings test split), còn `first10.wav` đo trên audio v5 **chưa từng thấy** — CER cải thiện ở H6 có thể là quá khớp miền train hẹp, không phải cải thiện chất lượng phiên âm nói chung. Chưa kết luận dứt khoát, cần ghi vào báo cáo cả hai số thay vì chọn một. Phần (b) vẫn giữ nguyên điều kiện chạy (chờ xác nhận hồi quy — nay đã có, mạnh hơn) | (a) done — N3 đóng |
...
| **H4** | **Bịt điểm mù số 4 — so run mới với model đang chạy production.** Đây là **row quan trọng nhất của plan**, vì nó là lỗi duy nhất chắc chắn và đúng bất kể nguyên nhân hành vi là gì. Hai phần: (a) `scripts/merge_and_push.py` nhận thêm một tham chiếu tới run/adapter **đang chạy production** và raise nếu ứng viên hồi quy so với nó — cạnh `check_provenance`, đúng nguyên tắc "mọi điều kiện đều kiểm trước khi push, không giả định" mà script đã có; (b) tier mới trong `src/gate.py` dùng `english_token_retention` của H3. Phần (b) chờ H6 để biết slice nào được quyền chặn push | CPU | H3, và (b) chờ H6 | Hàm check mới + raise khi hồi quy | Chạy lại trên adapter v4-mixed-r16 với tham chiếu là v3-r16 phải **raise** — trên 426 segment synthetic dùng chung, 0.0258 so với 0.0171 là hồi quy 51% tương đối. Không raise = check không hoạt động. **Không dùng segment không dùng chung**: khoá join là `(meeting_id, segment_id)`, `ref` phải trùng từng byte, nếu không thì đang so hai bộ test khác nhau (chi tiết ở H3) | **(a) và (b) done.** (a) `check_no_regression_vs_production` trong [scripts/merge_and_push.py](scripts/merge_and_push.py#L48) — join `(meeting_id, segment_id)`, raise nếu `ref` lệch giữa hai file, raise nếu CER ứng viên tệ hơn production trên tập chung; wired vào `main()` qua `--production-predictions` (tuỳ chọn), chạy **trước** bước merge fp32 CPU tốn thời gian. Chạy thật trên `Outputs/v4-mixed-r16/...` (ứng viên) so với `Outputs/v3-r16/...` (production) raise đúng, candidate cer=0.0258 vs production cer=0.0171 trên 426 segment chung — khớp số H3. (b) sau khi H6 xác nhận retention là tín hiệu chính (2026-08-17): thêm `english_token_retention` vào `_score_by_source` trong [src/gate.py](src/gate.py#L124) — mỗi source slice của tier1_in_domain giờ có `retention`/`retention_baseline`, và `retention_pass` (candidate không được tụt quá `cfg.gates.max_retention_regression_pp` so với baseline, mặc định 0.0 — cùng độ nghiêm với `real_cer_regression_pp`) khi cả hai phía có token tiếng Anh để đo (`n_candidates>0`; slice không có token nào thì bỏ qua `retention_pass`, không tính là fail). `tier1_in_domain.pass` giờ đòi cả `retention_pass` không False ở mọi slice, thêm vào rule CER cũ. Field mới `max_retention_regression_pp` trong `src/config.py` Gates + `configs/experiment.yaml`. 6 test mới trong `tests/test_gate.py`. Full suite **169 pass**. Field mới, áp dụng cho run tương lai — không hồi tố v4-mixed-r16 đã publish |
...
D3~~ — **cổng này không áp được nguyên văn.** H6 ra kết quả phân đôi (CER 0.0764 vs 0.1205 nói v5
tốt hơn; retention 0.7094 vs 0.7746 nói v5 tệ hơn, đúng chiều triệu chứng `team`→`tim` gốc). Người
dùng quyết 2026-08-17: **retention là tín hiệu chính** vì nó khớp đúng triệu chứng production được
báo cáo, còn CER không bắt được kiểu lỗi này (đó là lý do H3 tồn tại). Theo quyết định đó: **H4(b)
và D1 tiếp tục**, **D3 tạm hoãn** (CER đã cải thiện nên chưa rõ đổi tỉ lệ trộn có thật sự cần,
6.5h GPU không đáng đổ vào lúc chưa chắc).

**Cập nhật ngay sau đó, cùng ngày — lý do "CER đã cải thiện" phía trên không còn đứng vững.**
H2(a) đo trên `first10.wav` (audio thật, held-out khỏi cả hai model, xem row H2) cho **cả CER lẫn
retention cùng chiều "v5 tệ hơn"** (0.2818 vs 0.2461 / 0.5128 vs 0.7179). H6 và H2(a) không mâu
thuẫn nếu đọc đúng phạm vi: H6 đo trên slice **thuộc 7 meeting train** của v5, H2(a) đo trên
audio **ngoài** tập train của cả hai. Nguyên nhân khả dĩ, xếp theo độ tin cậy bằng chứng — **chưa
cái nào chứng minh dứt điểm, N2 vẫn mở**:

1. **Quá khớp vùng train hẹp (khả năng cao nhất).** `youtube-meetings` chỉ 7 recording, 3,46 giờ /
   790 segment (row D2), toàn "phỏng vấn tuyển dụng kỹ thuật" (dòng 263). CER tốt lên đúng trên
   slice thuộc 7 bản ghi đó (H6), tệ đi trên giọng/chủ đề mới (`first10.wav`, video `E5dAymt68-0`,
   không nằm trong 7 meeting train, đối chiếu `dataset/youtube-meetings/raw/`).
2. **Từ vựng tiếng Anh trong nhãn train hẹp theo đúng chủ đề 7 bản ghi đó.** Phiên soát thêm đúng
   `system`(38) `grab`(34) `leetcode`(34) `design`(21) `backend`(14) `engineer`(13) — từ vựng
   phỏng vấn tuyển dụng. `first10.wav` là webinar chia sẻ kinh nghiệm, loanword khác hẳn (`google`,
   `webinar`, `client`, `workspace`) — không được củng cố trong fine-tune, dễ bị thay bằng từ đồng
   âm tiếng Việt (`team`→`tim`, `build`→`bill`). Tương quan, chưa phải cơ chế đã chứng minh (N2).

   **Kiểm trực tiếp trên `first10.wav` (2026-08-17, không tốn GPU) — KHÔNG tách gọn được như kỳ
   vọng.** Đối chiếu từng chỗ mất trong ref với hyp5 quanh cùng mốc thời gian: **4 kiểu lỗi trộn
   lẫn**, không phải một cơ chế. (1) Thay bằng từ tiếng Việt khác nghĩa thật — `team`→`phim`
   (t=182s), cùng kiểu `team`→`tim` gốc. (2) Nghe nhầm **tên riêng** thành từ vô nghĩa —
   `Engineer Pro`→`nga` (t=26s), không phải đồng âm, là lỗi nhận dạng brand name. (3) Hallucinate
   từ lạ không tồn tại — `client`→`lion`/`ờjohn` (t=172s, 303s). (4) Lỗi chính tả nhẹ bị
   `english_token_retention` tính nhầm thành "mất" — `webinar`→`webina` (thiếu đúng 1 ký tự cuối,
   vẫn đang cố phát âm đúng từ tiếng Anh, không phải quên từ). `google` vẫn giữ đúng ở nhiều chỗ
   khác trong cùng file, không mất toàn bộ. **Không có "nút sửa" đơn nhất** — D3 (đổi tỉ lệ trộn)
   không chắc sửa được vì chưa rõ đang nhắm đúng lỗi nào trong 4 loại trên.
3. **Không phải do độ lớn adapter (loại trừ).** v5 dùng λ=0.25 (nhẹ hơn v4's λ=0.5) mà vẫn tệ hơn
   trên `first10.wav` — nếu chỉ là "adapter mạnh quá thì lệch", λ nhẹ hơn phải lệch *ít* hơn, không
   phải nhiều hơn. Vấn đề nằm ở **nội dung** dữ liệu trộn vào, không phải **lượng**.
4. **Casing nhãn — đóng góp thật nhưng nhỏ (đã đo, không đề cao).** Nhãn YouTube 0 chữ hoa/178.075
   ký tự (`review_youtube.py:78`, quy ước cố ý). v4 tự nó cũng chỉ xuất 15 hoa/8.515 ký tự.
5. **Chưa đo, không loại trừ:** đặc tính âm thanh 7 bản ghi train (thiết bị, phòng, kiểu ghi) khác
   `first10.wav` thế nào — hiện chỉ suy luận từ (1), chưa đo trực tiếp.

**Hệ quả cho D3:** lý do "tạm hoãn" phía trên dựa trên H6 một mình đã không còn đúng — nguyên nhân
số 1 và 2 đều chỉ thẳng vào "train hẹp cả về giọng lẫn từ vựng", đúng thứ D3 (đổi tỉ lệ trộn) và
D2 (thêm video thật) nhắm sửa. Cần người dùng quyết lại có chạy D3/D2 ngay hay không, xem cuối
file.

**Đo thêm trước khi quyết (người dùng chọn 2026-08-17) — kết quả PHỦ ĐỊNH giả thuyết 1+2 ở trên.**
`Outputs/{v3-r16,v4-mixed-r16}/audit/predictions_tier4a_real.csv` (264 chunk chung, `ref` khớp
từng byte) đã có sẵn từ H3 nhưng chưa nhìn breakdown — tier4a = `real-meetings-bench`, **cũng
held-out khỏi train của cả hai model**, nội dung "ML/technical walkthrough" + "student research
presentation (VAE)" (PROJECT_CORE.md §4), khác hẳn cả từ vựng phỏng vấn tuyển dụng (7 meeting
train) lẫn từ vựng webinar (`first10.wav`). Kết quả trên **627 candidate** (đủ lớn, không phải
nhiễu): CER v3=0.4801 vs v4mix=**0.4395** (v5 tốt hơn), retention v3=0.4083 vs v4mix=**0.4545**
(v5 tốt hơn) — từ mất nhiều nhất `sub` `layer` `self` `encoder` `multi` `attention` `transformer`
`vae` `forward` `norm` `clip`, ở CẢ HAI model gần như đều nhau, không lệch hẳn về phía v5.

**Ba bộ audio thật, ba kết quả khác nhau, không chia nổi theo "trong train / ngoài train":**
...
| `first10.wav` (webinar) | không | v5 tệ hơn | v5 tệ hơn |

Giả thuyết 1+2 (quá khớp vùng train hẹp / từ vựng train hẹp) dự đoán sai cho tier4a — domain đó
cũng ngoài train mà v5 vẫn tốt hơn cả hai chỉ số, mẫu đủ lớn để không phải nhiễu. **Hạ xuống "chưa
loại trừ nhưng không đủ giải thích".** Chưa xác định được biến nào phân tách đúng cả ba kết quả —
có thể liên quan tới kiểu lỗi khác nhau giữa "mất token hiếm hoàn toàn không có gì cạnh tranh"
(tier4a: `encoder`/`transformer` không có từ đồng âm tiếng Việt) và "bị thay bằng từ đồng âm tiếng
Việt" (`first10.wav`/production gốc: `team`→`tim`, `build`→`bill`) — nhưng đây vẫn là suy đoán,
chưa đo. **Bằng chứng hiện có mâu thuẫn nhau, chưa đủ để quyết chạy D3 (6.5h GPU) hay không** — cần
nói rõ điều này trong báo cáo, không chọn một trong ba số để kể câu chuyện gọn.

**Suy đoán trên cũng bị bác (2026-08-17, kiểm trực tiếp trên `first10.wav`, xem đoạn "Kiểm cơ chế"
ở gạch đầu dòng 2 phía trên).** Không phải "đồng âm vs mất hẳn" — quan sát được **4 kiểu lỗi trộn
lẫn trong cùng một file**: thay đồng âm thật (`team`→`phim`), nghe nhầm tên riêng thành rác
(`Engineer Pro`→`nga`), hallucinate từ lạ (`client`→`lion`/`ờjohn`), và lỗi chính tả nhẹ bị metric
tính nhầm là mất (`webinar`→`webina`). Không có nguyên nhân đơn nhất, do đó **không có "nút sửa"
đơn nhất** — D3 (đổi tỉ lệ trộn) là đặt cược vào một nguyên nhân chưa xác định. **Quyết định:
